...
ebr_volume_label:           db 'boot       '      ; 11 bytes, padded with spaces
...
.after:
  ...
  ; compute size of root directory = (32 * number_of_entriess) / bytes_per_sector
...
.load_kernel_loop:
  ...
  ; hardcoded 31 as value for this specific floppy disk type. change as appropriate
...
.halt:
  cli      ; disable interrupts, this way CPU can't get out of "halt" state
  hlt      ; jump unconditionally to a location
...
.retry:
  pusha             ; save (push) all registers, we don't know what bios modifies
  ...
  dec di
  test di, di
  jnz .retry        ; jump to .retry if not zero
...
; disk_resset: Resets disk controller
; Parameters:
;   - dl: drive number
disk_reset:
  pusha
...
;msg_hello: db 'cOS', ENDL, 0
msg_loading:            db 'Loading...', ENDL, 0
msg_read_failed:        db 'Read from disk filed!', ENDL, 0
